...
def test_symbol(data, num_partitions_to_keep=10, fixed_gamma=0.05, fixed_epsilon=0.001, use_random_partition=0, verbose=0, dump_partition_result=0, params=None):
    ...
    f_out = open(r'..\output\trading-7\%s_%d.csv' % (symbol, data.source), 'w')
    print >>f_out, 'date_str, positions_size, positions_price, cur_size, cur_price, unrealized pnl, pnl, balance, equity, predicted_value,  close_size, close_reason, trade_size, holding_period, stoploss, change, atr, predicted_value-cur_price, size_per_trade'
    ...
    trades = []
    initial_buying_power = int(avg_price * 10000) / 10000 * 10000
    ...
    for svm_line, extended_svm_line in zip(svm_lines_testing, extended_svm_lines_testing):
        ...
        size_per_trade = int(min(initial_buying_power * 0.1 / avg_price, initial_buying_power * 0.05 / atr)) / 10 * 10
        if size_per_trade <= 0:
            size_per_trade = 1
        ...
        print >>f_out, ','.join(map(str, [date_str, positions_size, positions_price, cur_size, cur_price, (cur_price - positions_price) * (positions_size), pnl, balance, equity, predicted_value,  close_size, close_reason, cur_size-close_size, holding_period, stoploss, change, atr, predicted_value-cur_price, size_per_trade]))
    print '-' * 20
    f_out.close()
    if total_trades == 0:
        total_trades = 1
...
def is_processed(symbol):
    file2 = r'..\output\trading-7\%s_0.csv' % (symbol)
    try:
...
if __name__ == '__main__':
    symbols = ['AORD', 'DJI', 'HSI', 'KS11', 'Nikkei225']
    ...
    for symbol in symbols:
        try:
            ...
            svm_data = SvmData(symbol, SvmData.YAHOO)
