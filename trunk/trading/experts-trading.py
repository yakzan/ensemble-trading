...
# prediction using SVM.
    ...
    positions_price = 0
    pnl = 0
    holding_period = data.time_delay * 3

    normal_price = extended_svm_lines_testing[0][-2]
    size_per_trade = int(initial_buying_power * 0.005 / extended_svm_lines_testing[0][-2]) / 10 * 10
    if size_per_trade > 500:
        size_per_trade = 500
    if size_per_trade <= 0:
        size_per_trade = 1
    ...
    change = data.scaling_map[1] / 10000
    total_trades = 0
    ...
    for svm_line, extended_svm_line in zip(svm_lines_testing, extended_svm_lines_testing):
        ...
        if data.source == SvmData.ONE_MIN:
            d = bar_date / 10000
            t = bar_date % 10000
        ...
        for i in range(len(trades)-1, -1, -1):
            ...
            if old_holding_period <= 1 or \
                (old_size > 0 and cur_price <= stoploss) or \
                (old_size < 0 and cur_price >= stoploss):
                cur_size -= old_size
                del trades[i]
            else:
                trades[i][1] -= 1
        ...
        if positions_size > 0: # was long
            if cur_size > 0: # long again
                ...
                pnl += (cur_price - positions_price) * abs(cur_size)
            ...
            if cur_size < 0: # short again
                ...
                pnl += (positions_price - cur_price) * cur_size
        else:
        ...
        print >>f_out, ','.join(map(str, [date_str, pnl, positions_size, positions_price, cur_size, close_size, cur_size-close_size, cur_price, predicted_value]))
    print '-' * 20
    f_out.close()
...
if __name__ == '__main__':
    ...
    for interval in [10, 15]:
        for symbol in ['INTC']: #['GLD', 'AMZN', 'DIA', 'BIDU', 'GOOG', 'IBM', 'GS', 'INTC', 'MSFT']:
    ...
    for symbol in []: # ['AAPL', 'MSFT', 'HSI', 'GE', 'NASDAQ']: # 'DJI',  'AORD', 'KS11', 'Nikkei225'
        print 'testing symbol:', symbol
