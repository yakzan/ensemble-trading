...
def test_symbol(data, num_partitions_to_keep=10, fixed_gamma=0.05, fixed_epsilon=0.001, use_random_partition=0, verbose=0, dump_partition_result=0, params=None):
    ...
    f_out = open(r'..\output\trading-5\%s_%d_%d.csv' % (symbol, data.source, data.interval), 'w')
    ...
    size_per_trade = int(initial_buying_power * 0.5 / normal_price) / 10 * 10
    if size_per_trade > 5000:
        size_per_trade = 5000
    if size_per_trade <= 0:
        size_per_trade = 1
        initial_buying_power = normal_price * 5000
        buying_power = initial_buying_power
    max_size_per_trade = size_per_trade * 10

    changes = [abs(line[-3] - line[-2]) / line[-2] for line in extended_svm_lines]
    max_changes = max(changes)
    #change = data.scaling_map[1] / 500000
    change = max_changes / 1000
    ...
    for svm_line, extended_svm_line in zip(svm_lines_testing, extended_svm_lines_testing):
        ...
        if cur_size-close_size != 0:
            if cur_size-close_size > 0: # buy
                stoploss = cur_price * 0.95 #cur_price - abs(predicted_value - cur_price)
            else: # sell
                stoploss = cur_price * 1.05 # cur_price + abs(predicted_value - cur_price)
        ...
        print >>f_out, ','.join(map(str, [date_str, positions_size, positions_price, cur_size, cur_price, (cur_price - positions_price) * (positions_size), pnl, equity, predicted_value,  close_size, close_reason, cur_size-close_size, holding_period, stoploss]))
    print '-' * 20
    f_out.close()
...
def is_processed(symbol, interval):
    file2 = r'../output/trading-5/%s_3_%d.csv' % (symbol, interval)
    try:
...
if __name__ == '__main__':
    symbol_list_part = 0
    if len(sys.argv) > 1:
        symbol_list_part = int(sys.argv[1])
    print symbol_list_part
    ...
    if symbol_list_part > 0:
        len_symbols = len(flist)
        flist = flist[(len_symbols / 4 + 1) * (symbol_list_part-1): (len_symbols / 4 + 1) * symbol_list_part]
        print (len_symbols / 4 + 1) * (symbol_list_part-1), (len_symbols / 4 + 1) * symbol_list_part
