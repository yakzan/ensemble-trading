...
class ShortOneMinBar:
    def __init__(self,  _date, _time, _open,  _high,  _low,  _close,  _volume):
        self.date, self.close, self.volume = \
            _date * 10000 + _time, _close, _volume
...
class SvmData:
    ...
    def __init__(self, symbol, source, interval=1, first_day=0, data_file=None):
        self.source = source
        self.symbol = symbol
        self.set_settings()
        ...
        if source == SvmData.YAHOO:
            if data_file is None:
                data_file = get_yahoo_file(symbol)
            self.bars = yahoo_to_bars(data_file)
        elif source == SvmData.ONE_MIN:
            if data_file is None:
                data_file = get_onemin_file(symbol)
            self.bars = onemin_to_bars(data_file, first_day)
        elif source == SvmData.ONE_MIN_COMP:
            if data_file is None:
                data_file = get_onemincomp_file(symbol)
            self.bars = onemincomp_to_bars(data_file, first_day)
        elif source == SvmData.FOREX:
            if data_file is None:
                data_file = get_forex_file(symbol)
            self.bars = forex_to_bars(data_file)
