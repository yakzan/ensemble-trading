...
class PositionManager:
    ...
    def handle_trade_signal(self, atr, true_range, avg_price, predicted_value, cur_price, cur_time):
        ...
        if diff > 0 and diff >= min_change and diff < max_change:
            ...
            stoploss   = ceil ((buy_price - gap) * 100 - 0.001) / 100
            takeprofit = floor((buy_price + gap) * 100 + 0.001) / 100
        elif diff < 0 and abs(diff) >= min_change and abs(diff) < max_change:
            # predicted DOWN, then sell
            cur_size -= size_per_trade
            stoploss   = floor((sell_price + gap) * 100 - 0.001) / 100
            takeprofit = ceil ((sell_price - gap) * 100 + 0.001) / 100
        ...
        cur_pnl = 0
        if comment:
            comment += ' true_range=%.4f' % (true_range)
...
class EnsemblePredictor:
    def __init__(self, first_day, symbol, interval=1, delay=1, dimension=5, gamma=0.0001, epsilon=0.001, num_partitions=200):
        self.symbol, self.interval, self.delay, self.dimension = symbol, interval, delay, dimension
    ...
    def train(self):
        ...
        print 'loading bars', symbol
        self.svm_data = SvmData(symbol, SvmData.ONE_MIN, self.interval, self.first_day)
        self.svm_data.set_settings(portion_training=1)
        self.svm_data.set_dimension_delay(self.dimension, self.delay)
        ...
        if 1:
            self.lower_atr = 0.001
            self.upper_atr = 0.010
        else:
    ...
    def update_with_bat(self, bat):
        ...
        true_range = abs(extended_svm_line[-7])
        #if atr < 0.015 or atr > 0.070: return
        if atr >= self.lower_atr and atr <= self.upper_atr and true_range < 0.03:
            self.position_manager.handle_trade_signal(atr, true_range, self.avg_price, predicted_value, price, cur_time)
        else:
            self.position_manager.write_line(cur_time, price, predicted_value, atr)
        ...
        try:
            ...
            while True:
                ...
                if dataLen <= 8:
                    continue
                if is_first:
                    print 'market data receiver works!!!!!!!!!!!!!!!!!!!'
                    is_first = False
                should_write = 0
...
def main():
    global ensemble_predictors
    global tradebot_client
    global force_instance_num

    if len(sys.argv) > 2:
        force_instance_num = int(sys.argv[2]) % total_instances
    ...
    if force_instance_num != -1:
        ensemble_predictors = main_worker(timestamp, force_instance_num)
    else:
