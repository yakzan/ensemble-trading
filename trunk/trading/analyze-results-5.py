...
def analyze_result(symbol, filename, interval, desc='DESC'):
    ...
    for line in f:
        ...
        if not f3:
            f3 = open('../output/plot-5/%s_%d.txt' % (symbol, interval), 'w')
    ...
    cmd = '''gnuplot -e "set terminal png size 1800,800; set output '../output/plot-5/%s_%d.jpg'; set y2tics; set tics out; set tics nomirror; unset xtics; set y2range [100000:300000];plot '../output/plot-5/%s_%d.txt' using 2 with l lc 5 title 'price', '' using 4 with l lc 4 title 'predicted', '' using 3 with l lc -1 title 'equity' axes x1y2, 200000 axes x1y2 title '%s' " ''' % (symbol, interval, symbol, interval, result_line2)
    #print cmd
    os.system(cmd)

def main():
    print 'symbol\tPNL\tTRADES\tPNL/TRADE\tMAX_DRAWDOWN\tMAX_EQUITY\tDESC'
    for file in glob.glob(r'd:\chimen-snippets\prediction\branches\over-embedding\output\trading-5\*.csv'):
        ...
        analyze_result(symbol, file, interval, 'interval=%d, 0.8 for training, holding 10, stoploss' % interval)
...
def main1():
    ...
    for symbol in ['XLP']:
        interval = 10
        #print file, symbol, interval
        file = r'd:\chimen-snippets\prediction\branches\over-embedding\output\trading-5\%s_3_%d.csv' % (symbol, interval)
        analyze_result(symbol, file, interval, 'interval=%d, 0.8 for training, holding 10, stoploss' % interval)
